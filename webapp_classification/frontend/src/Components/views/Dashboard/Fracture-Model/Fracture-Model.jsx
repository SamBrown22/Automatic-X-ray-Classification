...
import './Fracture-Model.css'; // Add styles in ClassificationModel.css
import Sidebar from '../../../Dashboard-Sidebar/Sidebar.jsx';
import { saveAs } from 'file-saver';
import { ToastContainer, toast } from 'react-toastify';

const FractureModel = ({onLogout, userData, setUserData}) => {
  ...
  const loadModel = async () => {
    try {
      const loadedModel = await tf.loadLayersModel('/models/Fracture-JSModel/model.json');
      ...
      try {
        ...
        const promises = selectedImages.map(async (imageBlobUrl, index) => {
          try {
            ...
            formData.append('image', blob, `${allPredictions[index] > 0.5 ? 'Fracture' : 'Not Fractured'}.jpg`);
          } catch (error) {
  ...
  const handleDownload = () => {
    if (predictions.length === selectedImages.length){
      ...
      .map((fileName, i) => `${fileName}: ${predictions[i] > 0.5 ? 'Fractured' : 'Not Fractured'}`)
      .join('\n');
  ...
  return (
    ...
    <div className="fracture-model">

      {/* Div to hold title and image */}
      <div className='fracture-heading-container'>
        <p className="fracture-heading">Fracture Classification Model</p>
      </div>

      {/* Input container for selecting an image file */}
      <div className="fracture-input-container">
        {/* Choose Image button */}
        <label htmlFor="fracture-file-input" id="fracture-file-input-label">Choose an image</label>
        <input id="fracture-file-input" type="file" accept="image/*" ref={imageRef} onChange={handleImageUpload} multiple/>

        {/* Predict button */}
        {selectedImages.length > 0 && (
          <button className='fracture-predict-btn' onClick={handlePredictionButtonClick} disabled={loading}>
      ...
      {selectedImages.length > 0 && (
        <div className="fracture-interact-container">
          {selectedImages.map((image, index) => (
            <div className='fracture-image-selected-container' key={index}>
              <img className="fracture-selected-image" src={image} alt={`Selected ${index}`} />
              {selectedFileNames[index] && (
                <div className='fracture-file-info'>
                  <p className="fracture-file-name">{`${selectedFileNames[index]}`}</p>
                  <button className='fracture-remove-button' onClick={() => removeImage(index)}>
                    <svg
      ...
      <div className='fracture-prediction-container'>
            {/* Display loading indicator if predictions are in progress */}
            {loading && <p className="fracture-loading-indicator">Waiting for predictions...</p>}
            
            {/* Display predictions if available */}
            {predictions.length === 1 && !loading && (
              <div>
                <p className='fracture-prediction-title'>The Model predicts: </p>
                <p className='fracture-prediction-content'>{`Probability that the Image contains a Fracture: ${(predictions[predictions.length-1] * 100).toFixed(2)}%`}</p>
                <p className='fracture-prediction-content'>{`Model Prediction: ${predictions[predictions.length-1] > 0.5 ? 'Fractured !' : 'Not Fractured'}`}</p>
              </div>
            )}
            ...
            {predictions.length > 1 && (
              <div>
                <button className='fracture-predict-btn' onClick={handleDownload}>
                  Download
                </button>
              </div>
            )}
